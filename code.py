...
for j in range(len(os.listdir("uploadimage"))):
  display = ''
  image = Image.open(os.path.join('uploadimage', os.listdir('uploadimage')[j]))
  ...
  for i in range(len(pixels)): 
    ...
    if count == image.size[0]//(image.size[0]//50):
      display+='\n'
      count = 0
  with open("result/result"+"image"+str(j)+'.txt', "w") as file:
      file.write(display)
